...
i=17; j=i+1;
fprintf(FID,'Homeowner ($\\gamma^{(h)}_8$) & %.3f%s & %.3f%s & %.3f%s & %.3f%s & %.3f%s & %.3f%s & %.3f%s & %.3f%s & %.3f%s \\\\ \n' , est(i,1),stars{i,1} ,est(i,2),stars{i,2} ,est(i,3),stars{i,3} , est(i,4),stars{i,4},  est(i,5),stars{i,5} ,est(i,6),stars{i,6} , est(i,7),stars{i,7} , est(i,8),stars{i,8} ,est(i,9),stars{i,9}  );
fprintf(FID,' & (%.3f) & (%.3f) & (%.3f)& (%.3f) & (%.3f) & (%.3f)& (%.3f) & (%.3f) & (%.3f) \\\\ \\bottomrule\n' , est(j,1), est(j,2),est(j,3), est(j,4), est(j,5),est(j,6), est(j,7), est(j,8),est(j,9));
...
fprintf(FID,'\\item The  estimates reported refer to the panel regressions  $\\hat{\\pi}^e_{i,t+h|t}=\\beta^{(h)} x_{i t}+\\mathbf{z}_{i}\\prime\\mathbf{\\gamma }^{(h)}+\\alpha_i^{(h)}+\\varepsilon_{i,t+h}$  using an unbalanced panel of 3,325 respondents over 11 months,\\\\  March 2012 to January 2013.\n');
fprintf(FID,'\\item $N=20,663$, $T_{min}=1$, $T_{p25}=3$, $T_{p50}=5$, $T_{p75}=10$,    $T_{max}=11$\n');
